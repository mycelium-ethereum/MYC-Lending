...
contract LentMycReader {
     ...
     */
    function getCurrentCycleInfo(address lentMyc_, uint256 ethRewards)
        external
        view
     ...
     */
    function _getCurrentCycleInfo(address lentMyc_, uint256 ethRewards)
        internal
        view
        returns (CycleInfo memory)
    {
        ...
        uint256 currentCyclesEthRewardsPerShare = (ethRewards + dust)
            .divWadDown(currentCycleSupply);
     ...
     */
    function getCycleInfo(address lentMyc_, uint256 cycle)
        external
        view
        returns (CycleInfo memory)
    {
        ...
        uint256 previousCumaltiveEthRewardsPerShare = lentMyc
            .cycleCumulativeEthRewards(cycle - 1);
        uint256 cyclesCumaltiveEthRewardsPerShare = lentMyc
            .cycleCumulativeEthRewards(cycle);
        uint256 cyclesEthRewardsPerShare = previousCumaltiveEthRewardsPerShare -
            cyclesCumaltiveEthRewardsPerShare;

        (uint256 cycleSupply, uint256 cycleMyc) = lentMyc.cycleSharesAndAssets(
            cycle
        );

        if (fetchingCurrentCycle) {
            uint256 lastCyclesEthRewards = cyclesEthRewardsPerShare.mulWadDown(
                cycleSupply
            );
